...
import { STEP } from './hooks/interfaces';
...
const AuthScreen = ({ route }) => {
  ...
  const { currentStep, videoRef } = useAuthScreen();
    ...
    <Box style={{ flex: 1 }}>
      ...
      <Box style={Styles.absoluteButtons}>
        ...
        <Box style={Styles.centerHorizontal}>
          <TextFactory>{"Don't have an account?"}</TextFactory> <Spacer size={12} isVertical={false} />
          <TouchableOpacity>
            <TextFactory>{'Sign Up now!'}</TextFactory>
          </TouchableOpacity>
        </Box>
      </Box>
    </Box>
