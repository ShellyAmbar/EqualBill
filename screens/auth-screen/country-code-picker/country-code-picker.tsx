...
const CountryCodePicker = ({ pickerStyle, modalStyle, textStyle, onPickedCode }: CountryCodePickerProps) => {
  const { isOpen, setIsOpen, selectedCountry, setSelectedCountry, countriesData, filteredCountriesData, setFilteredCountriesData } =
  ...
  const renderItem = item => {
    return (
      ...
      >
        <SvgUri viewBox="0 0 1000 500" uri={item.flag} width={30} height={30} style={Style.itemImage} />
  ...
  return (
    <>
      ...
      >
        {isOpen ? <DirectionUp width={20} height={20} /> : <DirectionDown width={20} height={20} />}

        <View style={Style.svg}>
          <SvgUri viewBox="0 0 1000 500" uri={selectedCountry.flag} width={30} height={30} style={{}} />
        </View>
        <Text style={{ ...Style.text, ...textStyle }}>{selectedCountry.alpha2Code}</Text>
      </TouchableOpacity>
            ...
            <View style={Style.modal}>
              ...
              <FilterBar
                ...
                onUpdateSearchTerm={t => {
                  const filteredList = countriesData.filter(item => item.name.toLowerCase().includes(t.toLowerCase()));
                  setFilteredCountriesData(filteredList);
                }}
              />
              <Spacer size={8} />
              <FlatList data={filteredCountriesData} renderItem={item => renderItem(item.item)} keyExtractor={item => item.flag} style={Style.list} />
            </View>
          </View>
