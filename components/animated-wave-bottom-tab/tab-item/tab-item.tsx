...
const TabItem: FC<TabProps> = ({ label, icon, index, activeIndex, onTabPress }) => {
  ...
  const tabStyle = useAnimatedStyle(() => {
    ...
    return {
      width: ICON_SIZE,
      height: ICON_SIZE,
      transform: [{ translateY: withTiming(translateY) }, { translateX: iconPositionX - ICON_SIZE / 2 }],
    };
  });
  const labelContainerStyle = useAnimatedStyle(() => {
    const translateY = animatedActiveIndex.value - 1 === index ? 36 : 100;
    return {
      transform: [{ translateY: withTiming(translateY) }, { translateX: labelPosition - LABEL_WIDTH / 2 }],
    };
  });
