...
import { Pressable, ScrollView, TouchableOpacity, TouchableOpacityProps, View, ViewProps } from 'react-native';
...
export const Box = forwardRef((props: boxProps, ref: any): JSX.Element => {
  ...
  return props.withoutFeedback ? (
    <Pressable {...props} ref={props.ref} style={props?.style && props?.style} onPress={props.onPress}>
      {props.children}
    </Pressable>
  ) : props.onPress ? (
    <TouchableOpacity {...props} ref={props.ref} style={props?.style && props?.style} onPress={props.onPress}>
      {props.children}
    </TouchableOpacity>
  ) : (
    BoxView
